...
public class TvScreenVertexConsumers {
    ...
    private static boolean hasSfx() {
        return !VistaLevelRenderer.isRenderingLiveFeed()
                ...
                // texture, which reads as the whole TV flashing. The feed pass already falls back
                // to entitySolid for the same reason.
                && !(CompatHandler.IRIS && IrisCompat.hasActiveShaderPack());
    }
